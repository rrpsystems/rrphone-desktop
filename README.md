...
Não é um fork de softphone existente: o motor SIP é o mesmo das demais
variantes do produto, e cada variante vive no seu próprio repositório.
...
# 3. mediastreamer2 com G.729, usando o shim versionado em cmake/
...
(`cmake/mediastreamer2-g729-shim.cmake`): os módulos `FindBV16`,
...
- **Padrão (sem senha):** a chave é injetada no build. Sem configurar nada, usa
  ...
  Para builds oficiais, crie `profile_key.txt` na raiz do repositório (ignorado pelo git):

  ```bash
  openssl rand -hex 32 > profile_key.txt
  ```
...
3. **liblinphone SDK (Windows)** — build pré-compilado da Belledonne Communications.
   ...
   **Já resolvido nesta cópia do projeto:** o zip `linphone-sdk-win64-5.5.17.zip`
   foi baixado e extraído em `third_party/linphone-sdk/win64/`
   ...
   `third_party/downloads/` guarda os zips originais (também
...
certo. A partir da raiz do repositório:
